...
function createHazards(levelIndex: number, width: number, shiftX: number): HazardPlacement[] {
  const tier = levelIndex < 6 ? "early" : levelIndex < 12 ? "mid" : "late";
  const isFinalLevel = levelIndex === stationConfigs.length - 1;
  const pattern = levelIndex % 5;
  const seatWidth = tier === "early" ? 132 : tier === "mid" ? 156 : 190;
  const seatDepth = tier === "early" ? 84 : tier === "mid" ? 92 : 104;
  ...
  if (isFinalLevel) {
    return [
      ...
      {
        id: `hazard-door-boss-${levelIndex}`,
        type: "door_slam",
        x: 1720 + shiftX,
        y: 72,
        width: 60,
  ...
  if (pattern === 0) {
    const damage = tier === "early" ? 7 : tier === "mid" ? 9 : 11;
    const activeMs = tier === "early" ? 460 : tier === "mid" ? 520 : 600;
    const cooldownMs = tier === "early" ? 1050 : tier === "mid" ? 900 : 760;
    const strength = tier === "early" ? -20 : tier === "mid" ? -30 : -40;
  ...
  if (pattern === 1) {
    const brakeStrength = tier === "early" ? -110 : tier === "mid" ? -155 : -210;
    const activeMs = tier === "early" ? 300 : tier === "mid" ? 380 : 460;
    const cooldownMs = tier === "early" ? 1600 : tier === "mid" ? 1350 : 1080;
  ...
  if (pattern === 2) {
    const pushWidth = tier === "early" ? 300 : tier === "mid" ? 360 : 420;
    const pushStrength = tier === "early" ? 140 : tier === "mid" ? 185 : 230;
    const cooldownMs = tier === "early" ? 1380 : tier === "mid" ? 1220 : 980;
  ...
  if (pattern === 3) {
    const clutterWidth = tier === "early" ? 140 : tier === "mid" ? 170 : 210;
    const clutterDepth = tier === "early" ? 64 : tier === "mid" ? 74 : 90;
    const slowdown = tier === "early" ? 0.58 : tier === "mid" ? 0.46 : 0.34;
